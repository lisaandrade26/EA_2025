...
cat("Ejercicio de Moneda - Distribución Geométrica(p = 0.5)\n")
cat("X: Número de sellos antes de la primera cara.\n\n")

k_fallos_a <- 4
prob_a <- dgeom(k_fallos_a, p_exito)

cat("a) P(necesitar 5 lanzamientos) = P(X = 4 fallos)\n")
cat("P(X = 4) =", prob_a, " (", round(prob_a * 100, 3), "%)\n\n")


k_min_fallos_b <- 3 # 3 fallos implican 4 lanzamientos totales.

prob_b <- (1 - p_exito)^k_min_fallos_b

cat("b) P(necesitar > 3 lanzamientos) = P(X >= 3 fallos)\n")
cat("P(X >= 3) =", prob_b, " (", round(prob_b * 100, 3), "%)\n\n")

media_geometrica <- (1 - p_exito) / p_exito
varianza_geometrica <- (1 - p_exito) / (p_exito^2)

cat("c) Parámetros Estadísticos (sobre el número de fracasos):\n")
cat("Media (E[X]):", media_geometrica, "\n")
cat("Varianza (Var[X]):", varianza_geometrica, "\n")

cat("El número esperado de lanzamientos totales es E[X] + 1 = 2.0\n")
```
...
cat("Ejercicio de Baloncesto - Distribución Geométrica (p = 0.70)\n")
cat("X: Número de fallos antes de la primera canasta.\n\n")

k_fallos_a <- 2
prob_a <- dgeom(k_fallos_a, p_exito)

cat("a) P(anotar en el 3er intento) = P(X = 2 fallos)\n")
cat("P(X = 2) =", round(prob_a, 5), "\n\n")


media_fracasos <- (1 - p_exito) / p_exito
media_intentos_totales <- media_fracasos + 1
varianza_fracasos <- (1 - p_exito) / (p_exito^2)

cat("b) Parámetros Estadísticos:\n")
cat("Media de Fracasos (E[X]):", round(media_fracasos, 4), "\n")
cat("Intentos Esperados (E[X]+1):", round(media_intentos_totales, 4), "\n")
cat("Varianza de Fracasos (Var[X]):", round(varianza_fracasos, 4), "\n\n")


k_inicio <- 1
k_fin <- 4
valores_k <- k_inicio:k_fin

probabilidades <- dgeom(valores_k, p_exito)
prob_c <- sum(probabilidades)


cat("c) P(necesitar entre 2 y 5 intentos) = P(1 <= X <= 4 fallos)\n")
cat("Suma de P(X=1) a P(X=4) =", round(prob_c, 5), "\n")
...
p_defecto <- 0.02 # Probabilidad de éxito 
p_no_defecto <- 1 - p_defecto 

cat("Ejercicio de Control de Calidad - Distribución Geométrica (p = 0.02) \n\n")

media_fracasos <- p_no_defecto / p_defecto
media_inspecciones_totales <- media_fracasos + 1

cat("a) Número esperado de inspecciones totales:\n")
cat("E[X] (Fracasos):", media_fracasos, "\n")
cat("E[X] + 1 (Inspecciones Totales):", media_inspecciones_totales, "\n\n")


k_fallos_b <- 100
# P(X >= k) = (1-p)^k
prob_b <- p_no_defecto^k_fallos_b

cat("b) P(Primera defectuosa después de la pieza 100) = P(X >= 100 fallos):\n")
cat("P(X >= 100) =", round(prob_b, 6), "\n\n")

s <- 50
t <- 20

prob_derecha <- p_no_defecto^t

prob_s_t <- p_no_defecto^(s + t)
# P(X >= s)
prob_s <- p_no_defecto^s

prob_izquierda <- prob_s_t / prob_s

cat("c) Demostración de Falta de Memoria (s=50, t=20):\n")
cat("Lado Izquierdo P(X >= 70 | X >= 50):", round(prob_izquierda, 6), "\n")
cat("Lado Derecho P(X >= 20):", round(prob_derecha, 6), "\n")
cat("¿Ambos lados son iguales?", abs(prob_izquierda - prob_derecha) < 1e-10, "\n")
