...
ggplot(data.frame(iteraciones = iteraciones_fallidas), aes(x = iteraciones)) +
  ...
  geom_vline(xintercept = media_iteraciones, color = "red", 
...
comparar_geo_vs_binneg <- function(r, p, n_sim) {
  ...
  data_comparacion <- data.frame(
    valores = c(suma_geometricas, binomial_negativa),
    metodo = rep(
      c(paste0("Suma de r=", r, " Geométricas"), 
        paste0("Binomial Negativa (r=", r, ")")), 
      each = n_sim
    )
  )
 
  plot_comparativo <- ggplot(data_comparacion, aes(x = valores, fill = metodo)) +
    geom_histogram(alpha = 0.8, bins = 30, color = "white", show.legend = FALSE) +
    facet_wrap(~metodo, ncol = 1, scales = "free_y") + 
    labs(
      title = "Verificación: Suma de Geométricas = Binomial Negativa",
      subtitle = paste("r =", r, ", p =", p, ", n_sim =", format(n_sim, big.mark = ",")),
      x = "Número de fracasos (k)",
      y = "Frecuencia (Conteo de simulación)"
    ) +
    theme_minimal() +
    theme(
...
grafico_resultado <- comparar_geo_vs_binneg(r = r_val, p = p_val, n_sim = n_sim_val)
...
cat("--- EJERCICIO DE BALONCESTO (BINOMIAL NEGATIVA)\n")
cat(paste0("Parámetros: r = ", r_exitos, " éxitos, p = ", p_exito, "\n\n"))

cat(" a) Modelo Teórico (Suma de Geométricas) \n")
cat("1. Sea X_i la variable aleatoria que cuenta el número de 'fracasos' (tiros fallados)\n")
cat("   antes de obtener el i-ésimo 'éxito' (tiro anotado).\n")
cat(paste0("2. Cada X_i sigue una distribución Geométrica: X_i ~ Geom(p = ", p_exito, ")\n"))
cat("3. Sea Y el número total de tiros fallados antes de obtener los r=8 éxitos.\n")
cat("4. Por la proposición, Y es la suma de r variables geométricas independientes:\n")
cat("   Y = X_1 + X_2 + ... + X_8\n")
cat(paste0("5. Por lo tanto, Y sigue una Distribución Binomial Negativa: Y ~ BinNeg(r = ", r_exitos, ", p = ", p_exito, ")\n\n"))


media_teorica <- r_exitos * (1 - p_exito) / p_exito

varianza_teorica <- r_exitos * (1 - p_exito) / (p_exito^2)

cat("--- b) Cálculo Teórico de Tiros Fallados ---\n")
cat(paste("Media (E[Y]):", round(media_teorica, 4), "\n"))
cat(paste("Varianza (Var[Y]):", round(varianza_teorica, 4), "\n\n"))


cat("--- c) Simulación de 1000 Sesiones ---\n")
set.seed(42) 

sesiones_simuladas <- replicate(n_sim, {
  sum(rgeom(r_exitos, prob = p_exito))
})

media_simulada <- mean(sesiones_simuladas)
varianza_simulada <- var(sesiones_simuladas)

tabla_comparativa <- data.frame(
  Estadistica = c("Media (Esperanza)", "Varianza"),
  Teorica = c(media_teorica, varianza_teorica),
  Simulada = c(media_simulada, varianza_simulada)
)

tabla_comparativa$Diferencia <- abs(tabla_comparativa$Teorica - tabla_comparativa$Simulada)

print(kable(tabla_comparativa, digits = 4,
            caption = "Comparación: Valores Teóricos vs. Simulación (n=1000)"))

data_hist <- data.frame(Fallos = sesiones_simuladas)

plot_simulacion <- ggplot(data_hist, aes(x = Fallos)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 1, 
                 fill = "deeppink", color = "white", alpha = 0.7) +
  geom_vline(xintercept = media_teorica, color = "navy", 
             linetype = "dashed", linewidth = 1.2) +
  labs(
    title = "Distribución Simulada de Tiros Fallados (n=1000)",
    subtitle = paste("r=8, p=0.70. Línea azul: Media Teórica =", round(media_teorica, 3)),
    x = "Número de Tiros Fallados (Y)",
    y = "Densidad"
  ) +
  theme_minimal()
...
k_fallos_a <- 12
prob_a <- dnbinom(x = k_fallos_a, size = r_exitos, prob = p_exito)

cat("--- a) P(X = 12 fallos) ---\n")
cat(paste("La probabilidad de tener exactamente 12 fallos es:", round(prob_a, 6)), "\n\n")

k_fallos_b <- 20

prob_b <- 1 - pnbinom(q = k_fallos_b, size = r_exitos, prob = p_exito)

cat("--- b) P(X > 20 fallos) ---\n")
cat(paste("La probabilidad de tener más de 20 fallos es:", round(prob_b, 6)), "\n\n")

media_fallos_teorica <- r_exitos * (1 - p_exito) / p_exito
costo_esperado <- media_fallos_teorica * costo_prueba

cat("--- c) Costo Esperado de Pruebas Fallidas ---\n")
cat(paste("Número esperado de fallos (E[X]):", round(media_fallos_teorica, 4), "\n"))
cat(paste("Costo por prueba: $", format(costo_prueba, big.mark = ","), "\n"))
cat(paste("Costo esperado total (E[X] * Costo): $", format(round(costo_esperado, 2), big.mark = ","), "\n"))
