...
datos_convergencia <- lapply(grados_libertad, function(gl) {

  muestra <- rchisq(n_max, df = gl)
  
  media_acumulada <- cummean(muestra)
  
  data.frame(
    iteracion = 1:n_max,
    media_muestral = media_acumulada,
    media_teorica = gl, 
    caso = factor(paste("Grados de Libertad n =", gl), 
                  levels = paste("Grados de Libertad n =", grados_libertad))
  )
}) %>% 
  bind_rows() %>% 
  filter(iteracion >= 100) 

ggplot(datos_convergencia, aes(x = iteracion, y = media_muestral)) +
  geom_line(color = "skyblue1", linewidth = 0.8) +
  
  geom_hline(aes(yintercept = media_teorica), 
             color = "orange", linetype = "dashed", linewidth = 1) +
  
  facet_wrap(~caso, scales = "free_y", ncol = 1) +
  
  labs(
    title = "Convergencia de la Media Muestral (Ley de los Grandes Números)",
    subtitle = "Distribución Ji-Cuadrada: La línea azul (muestral) converge a la roja (teórica)",
...
datos_var_conv <- lapply(grados_libertad, function(gl) {
  
  muestra <- rchisq(n_max, df = gl)
  
  n_seq <- 1:n_max
  sum_x <- cumsum(muestra)
  sum_sq_x <- cumsum(muestra^2)
  
  var_acumulada <- (sum_sq_x - (sum_x^2) / n_seq) / (n_seq - 1)
  
  data.frame(
    iteracion = n_seq,
    var_muestral = var_acumulada,
    var_teorica = 2 * gl, 
    caso = factor(paste("Grados de Libertad n =", gl),
                  levels = paste("Grados de Libertad n =", grados_libertad))
  )
  ...
  bind_rows() %>%
  filter(iteracion >= 100)
ggplot(datos_var_conv, aes(x = iteracion, y = var_muestral)) +

  geom_line(color = "green4", linewidth = 0.8) +
  
  geom_hline(aes(yintercept = var_teorica), 
             color = "sienna4", linetype = "dashed", linewidth = 1) +

  facet_wrap(~caso, scales = "free_y", ncol = 1) +
  
  labs(
    title = "Convergencia de la Varianza Muestral",
    subtitle = "Distribución Ji-Cuadrada: Convergencia hacia 2n",
    x = "Tamaño de la Muestra",
    y = "Varianza Muestral (S²)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", size = 14),
...
datos_hist <- lapply(grados_libertad, function(gl) {
  data.frame(
    valor = rchisq(n_grande, df = gl),
    caso = factor(paste("Grados de Libertad n =", gl), 
                  levels = paste("Grados de Libertad n =", grados_libertad))
  )
}) %>% bind_rows()

datos_teoricos <- lapply(grados_libertad, function(gl) {
  max_x <- qchisq(0.999, df = gl) 
  x_seq <- seq(0, max_x, length.out = 300)
  
  data.frame(
    x = x_seq,
    y = dchisq(x_seq, df = gl),
    caso = factor(paste("Grados de Libertad n =", gl), 
                  levels = paste("Grados de Libertad n =", grados_libertad))
  )
}) %>% bind_rows()
...
ggplot() +
  geom_histogram(data = datos_hist, aes(x = valor, y = after_stat(density)),
                 bins = 50, 
                 fill = "violet", 
                 color = "white", 
                 alpha = 0.6) +
  ...
  geom_line(data = datos_teoricos, aes(x = x, y = y),
            color = "gray2", linewidth = 1.2) +
  
  facet_wrap(~caso, scales = "free") +
  
  labs(
    title = "Ajuste de la Muestra a la Distribución Teórica",
    subtitle = paste("Comparación para muestra grande (n =", n_grande, ")"),
    x = "Valor de Ji-Cuadrada",
    y = "Densidad"
  ) +
  theme_minimal() +
  theme(
...
gl_poblacion <- 4 
n_simulaciones <- 2000 
mu_poblacion <- gl_poblacion
var_poblacion <- 2 * gl_poblacion

verificar_tcl_chi <- function(n_muestra) {

  medias <- replicate(n_simulaciones, mean(rchisq(n_muestra, df = gl_poblacion)))
  
  datos <- data.frame(valor = medias)
  
 
  sd_teorica_media <- sqrt(var_poblacion / n_muestra)
  ...
  p <- ggplot(datos, aes(x = valor)) +
    ...
    stat_function(fun = dnorm, 
                  args = list(mean = mu_poblacion, sd = sd_teorica_media), 
                  color = "black", 
                  linewidth = 1.1) +
    
    labs(
      title = paste("Tamaño de muestra n =", n_muestra),
      subtitle = paste("N(mu=", mu_poblacion, ", sigma=", round(sd_teorica_media, 2), ")"),
      x = "Media Muestral",
      y = "Densidad"
    ) +
    theme_minimal() +
...
p1 <- verificar_tcl_chi(5)  
p2 <- verificar_tcl_chi(30)  
p3 <- verificar_tcl_chi(100)  
p4 <- verificar_tcl_chi(500)

grid.arrange(p1, p2, p3, p4, ncol = 2, 
             top = "Verificación del TCL aplicado a una Ji-Cuadrada(4)")
```
...
matriz_z <- matrix(rnorm(n_sim * k), nrow = n_sim, ncol = k)
suma_cuadrados <- rowSums(matriz_z^2)

datos <- data.frame(valor = suma_cuadrados)
...
ggplot(datos, aes(x = valor)) +
  ...
  stat_function(fun = dchisq, 
                ...
                linewidth = 1.2) +
  labs(
    title = "Verificación de Aditividad: Suma de Normales al Cuadrado",
    subtitle = expression(paste("Histograma de ", sum(Z[i]^2), " vs Teórica ", chi^2, "(8)")),
    x = "Valor de la Suma",
    y = "Densidad"
  ) +
  theme_minimal() +
...
```{r}
set.seed(123)
n_valores <- c(10, 30, 50, 100)
n_sim <- 10000

graficar_aproximacion <- function(n) {
  
  datos <- data.frame(valor = rchisq(n_sim, df = n))
  ...
  ggplot(datos, aes(x = valor)) +
    geom_histogram(aes(y = after_stat(density)), 
                   ...
                   alpha = 0.7) +
    stat_function(fun = dnorm, 
                  args = list(mean = mu_teorica, sd = sd_teorica), 
                  color = "sienna2", 
                  linewidth = 1.2) +
    labs(
      title = paste("Grados de Libertad n =", n),
      subtitle = paste("Aprox. Normal(μ=", mu_teorica, ", σ=", round(sd_teorica, 2), ")"),
      x = "Valor",
      y = "Densidad"
    ) +
    theme_minimal() +
...
lista_graficos <- lapply(n_valores, graficar_aproximacion)

grid.arrange(grobs = lista_graficos, ncol = 2, 
             top = "Verificación de la Aproximación Normal para Chi-Cuadrada")
```
...
chi1 <- rchisq(n_sim, df = df1)
chi2 <- rchisq(n_sim, df = df2)
suma_chi <- chi1 + chi2

datos <- data.frame(valor = suma_chi)
...
ggplot(datos, aes(x = valor)) +
  geom_histogram(aes(y = after_stat(density)), 
                 ...
                 alpha = 0.7) +
  stat_function(fun = dchisq, 
                args = list(df = df_total), 
                color = "gray2", 
                linewidth = 1.2) +
  labs(
    title = "Verificación de Aditividad: Suma de dos variables Ji-Cuadrada",
    subtitle = paste("Histograma de Chi^2(", df1, ") + Chi^2(", df2, ") vs Teórica Chi^2(", df_total, ")"),
    x = "Valor de la Suma",
    y = "Densidad"
  ) +
  theme_minimal() +
