...
set.seed(42) 
df_param <- 5
tamanios <- c(100, 300, 1000)

mu_teorica <- 0 

var_teorica <- df_param / (df_param - 2)

tabla_resultados <- data.frame()

for(n in tamanios) {
  muestra <- rt(n, df = df_param)
  ...
  temp_df <- data.frame(
    ...
    "Media_Teórica" = mu_teorica,
    "Diferencia_Media" = abs(media_muestral - mu_teorica),
    "Varianza_Muestral" = var_muestral,
    "Varianza_Teórica" = var_teorica,
    "Diferencia_Var" = abs(var_muestral - var_teorica)
  )
  
  tabla_resultados <- rbind(tabla_resultados, temp_df)
}

kable(tabla_resultados, 
      digits = 4, 
      caption = "Comparación de Estadísticos: Muestral vs Teórico (t de Student, df=5)",
      align = 'c')
```
...
datos_grafico <- data.frame(
  valor = rt(n_grande, df = df_param)
)

ggplot(datos_grafico, aes(x = valor)) +
  ...
  stat_function(fun = dt, 
                args = list(df = df_param), 
                aes(color = "Teórica t(5)", linetype = "Teórica t(5)"),
                linewidth = 1.2) +
  
  stat_function(fun = dnorm, 
                aes(color = "Normal Estándar", linetype = "Normal Estándar"),
                linewidth = 1) +
  
  scale_color_manual(name = "Distribuciones", 
                     values = c("Teórica t(5)" = "navy", 
                                "Normal Estándar" = "red")) +
  scale_linetype_manual(name = "Distribuciones",
                        values = c("Teórica t(5)" = "solid", 
                                   "Normal Estándar" = "dashed")) +

  labs(
    title = "Comparación: Muestra t(5) vs Teórica vs Normal",
    subtitle = paste("Tamaño de muestra n =", n_grande, "| Grados de libertad =", df_param),
    x = "Valor",
    y = "Densidad"
  ) +
  theme_minimal() +
...
ggplot (df_exr) + 
  geom_histogram(aes(x=T, y = after_stat(density)), bins = 100, fill = "lightblue", color = "black", alpha = 0.7) + geom_function(fun = function (x) dt (x, df=n), color = "red", linewidth = 1.3) +
geom_function(fun = function (x) dnorm (x, df=n), color = "yellow", linewidth = 0.5)
```
...
Y <- rchisq(n_sim, df = df_param)        

T_simulada <- X / sqrt(Y / df_param)

datos <- data.frame(valor = T_simulada)
...
ggplot(datos, aes(x = valor)) +
  ...
  labs(
    title = "Validación de la construcción de la distribución t",
    subtitle = expression(paste("Histograma de ", T == frac(X, sqrt(Y/10)), " vs Densidad Teórica ", t(10))),
    x = "Valor de T",
    y = "Densidad"
  ) +
  theme_minimal() +
