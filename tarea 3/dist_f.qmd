...
p_a <- ggplot(df_a, aes(x = valor)) +
  ...
  labs(
    title = "Propiedad a): Cuadrado de t-Student",
    subtitle = paste0("Histograma de t(", n, ")^2 vs Curva Teórica F(1, ", n, ")"),
    x = "x",
    y = "Densidad"
  ) +
  theme_minimal()
...
p_b <- ggplot(df_b, aes(x = valor)) +
  ...
  stat_function(fun = df, 
                args = list(df1 = b, df2 = a), 
                color = "black", 
                linewidth = 1.2) +
  
  labs(
    title = "Propiedad b): Inverso de F",
    subtitle = paste0("Histograma de 1/X (donde X~F(", a, ",", b, ")) vs Curva Teórica F(", b, ",", a, ")"),
    x = "x",
    y = "Densidad"
  ) +
  theme_minimal()
...
chi_num <- rchisq(n_sim, df = a) 
chi_den <- rchisq(n_sim, df = b) 

y_sim_c <- (chi_num / a) / (chi_den / b)

df_c <- data.frame(valor = y_sim_c)

p_c <- ggplot(df_c, aes(x = valor)) +
  geom_histogram(aes(y = after_stat(density)), 
                 bins = 60, 
                 fill = "seagreen3", 
                 color = "white", 
  ...
  labs(
    title = "Propiedad c): Cociente de Chi-cuadradas",
    subtitle = paste0("Histograma de (X/a)/(Y/b) vs Curva Teórica F(", a, ",", b, ")"),
    x = "x",
    y = "Densidad"
  ) +
  theme_minimal()
