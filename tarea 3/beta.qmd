...
datos <- data.frame(
  n = 1:n_max,
  valor = rbeta(n_max, a, b)
) %>%
  mutate(
   
    media_muestral = cummean(valor)
  ) %>%
 
  filter(n >= 100)

ggplot(datos, aes(x = n, y = media_muestral)) +
  
  geom_line(color = "pink", linewidth = 0.8) +
  
  geom_hline(yintercept = media_teorica, color = "purple", linetype = "dashed", linewidth = 1) +
  
 
  labs(
    title = "Convergencia de la Media Muestral a la Media Teórica",
    subtitle = paste0("Distribución Beta(2, 5) | Media Teórica = ", round(media_teorica, 4)),
...
datos_var <- data.frame(
  ...
  mutate(
    sum_x = cumsum(valor),
    sum_sq_x = cumsum(valor^2),
    var_muestral = (sum_sq_x - (sum_x^2) / n) / (n - 1)
  ) %>%
  filter(n >= 100)

ggplot(datos_var, aes(x = n, y = var_muestral)) +
  # Línea de la varianza muestral
  geom_line(color = "deeppink", linewidth = 0.8) +
  
  geom_hline(yintercept = var_teorica, color = "red", linetype = "dashed", linewidth = 1) +
  
  labs(
    title = "Convergencia de la Varianza Muestral a la Teórica",
    subtitle = paste0("Distribución Beta(2, 5) | Varianza Teórica = ", round(var_teorica, 5)),
    x = "Tamaño de la muestra (n)",
...
datos_hist <- data.frame(
  x = rbeta(n, a, b)
)

ggplot(datos_hist, aes(x = x)) +
  ...
  labs(
    title = paste0("Histograma (n=", n, ") vs PDF Teórica"),
    subtitle = "La curva azul oscuro representa la distribución Beta(2, 5) exacta",
    x = "Valor (x)",
    y = "Densidad"
  ) +
...
generar_tcl <- function(tam_muestra) {
  medias <- numeric(n_simulaciones)
  for(i in 1:n_simulaciones) {
    medias[i] <- mean(rbeta(tam_muestra, a, b))
  }
  
  sd_tcl <- sqrt(var_teorica / tam_muestra)
  ...
  p <- ggplot(data.frame(x = medias), aes(x = x)) +
    ...
    stat_function(fun = dnorm, 
                  args = list(mean = mu_teorica, sd = sd_tcl), 
                  color = "red4", 
                  linewidth = 1) +
    
    labs(
      title = paste("n =", tam_muestra),
      subtitle = paste("Normal(μ=", round(mu_teorica, 2), ", σ=", round(sd_tcl, 3), ")"),
      x = "Media Muestral", 
      y = "Densidad"
    ) +
    theme_minimal() +
