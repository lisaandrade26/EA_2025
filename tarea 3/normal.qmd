...
alturas <- rnorm(n_estudiantes, mean = mu_altura, sd = sigma_altura)
...
print(paste("Mediana muestral:", mediana_muestral))
print(paste("Desviación estándar muestral:", sd_muestral))
```
3.  ¿Qué porcentaje de estudiantes mide más de 180 cm?
```{r}
porcentaje_simulado <- mean(alturas > 180) * 100

porcentaje_teorico <- (1 - pnorm(180, mean = 170, sd = 8)) * 100

print(paste("Porcentaje simulado (> 180cm):", round(porcentaje_simulado, 2), "%"))
print(paste("Porcentaje teórico (> 180cm):", round(porcentaje_teorico, 2), "%"))
```

4.  ¿Cuál es la altura del estudiante en el percentil 25?
```{r}
p25_muestral <- quantile(alturas, 0.25)
p25_teorico <- qnorm(0.25, mean = 170, sd = 8)

print(paste("Percentil 25 muestral:", round(p25_muestral, 2), "cm"))
print(paste("Percentil 25 teórico:", round(p25_teorico, 2), "cm"))
```

5.  Crea un histograma de la muestra con la densidad teórica superpuesta
```{r}
datos <- data.frame(altura = alturas)
...
ggplot(datos, aes(x = altura)) +
  ...
  stat_function(fun = dnorm, 
                args = list(mean = 170, sd = 8), 
                color = "red", 
                linewidth = 1.2) +
  labs(
    title = "Distribución de Alturas de Estudiantes",
    subtitle = "Histograma muestral vs Densidad Teórica N(170, 8)",
    x = "Altura (cm)",
    y = "Densidad"
  ) +
  theme_minimal() +
...
mediciones <- rnorm(n_mediciones, mean = mu_temp, sd = sigma_temp)
...
conteo_fuera <- sum(mediciones < limite_inferior | mediciones > limite_superior)
porcentaje_fuera <- (conteo_fuera / length(mediciones)) * 100

prob_teorica <- (pnorm(limite_inferior, mean = 350, sd = 5) + 
                 (1 - pnorm(limite_superior, mean = 350, sd = 5))) * 100

print(paste("Porcentaje fuera de especificación (simulado):", porcentaje_fuera, "%"))
print(paste("Porcentaje fuera de especificación (teórico):", round(prob_teorica, 4), "%"))
```
...
mediciones_descentradas <- rnorm(n_mediciones, mean = mu_nueva, sd = sigma_temp)

porcentaje_fuera_nuevo <- mean(mediciones_descentradas < limite_inferior | 
                               mediciones_descentradas > limite_superior) * 100

prob_teorica_nueva <- (pnorm(limite_inferior, mean = mu_nueva, sd = sigma_temp) + 
                       (1 - pnorm(limite_superior, mean = mu_nueva, sd = sigma_temp))) * 100

print(paste("Porcentaje fuera de especificación (mu=355):", porcentaje_fuera_nuevo, "%"))
print(paste("Porcentaje teórico fuera de especificación:", round(prob_teorica_nueva, 4), "%"))
```
...
alpha <- 1 - prob_objetivo
z_critico <- qnorm(1 - alpha/2)

sigma_requerida <- tolerancia / z_critico

print(paste("Z crítico:", round(z_critico, 4)))
print(paste("Sigma necesaria:", round(sigma_requerida, 4)))

prob_verificacion <- pnorm(360, mean = mu_objetivo, sd = sigma_requerida) - 
                     pnorm(340, mean = mu_objetivo, sd = sigma_requerida)

print(paste("Verificación de cobertura:", prob_verificacion))
```
...
prob_teorica <- 1 - pnorm(umbral, mean = mu_ventas, sd = sigma_ventas)

prob_simulada <- mean(ventas_mensuales > umbral)

print(paste("Probabilidad teórica (> 60,000):", round(prob_teorica, 4)))
print(paste("Probabilidad simulada en la muestra:", round(prob_simulada, 4)))
```
...
prob_teorica <- pnorm(limite, mean = mu_ventas, sd = sigma_ventas)

meses_esperados <- n_meses * prob_teorica

set.seed(123)
ventas_mensuales <- rnorm(n_meses, mean = mu_ventas, sd = sigma_ventas)
meses_observados <- sum(ventas_mensuales < limite)

print(paste("Probabilidad P(X < 30,000):", round(prob_teorica, 4)))
print(paste("Meses esperados (Teórico):", round(meses_esperados, 2)))
print(paste("Meses observados (Simulación):", meses_observados))
```
...
iqr_muestral <- IQR(ventas_mensuales)

q1_teorico <- qnorm(0.25, mean = mu_ventas, sd = sigma_ventas)
q3_teorico <- qnorm(0.75, mean = mu_ventas, sd = sigma_ventas)
iqr_teorico <- q3_teorico - q1_teorico

print(paste("IQR Muestral:", round(iqr_muestral, 2)))
print(paste("IQR Teórico:", round(iqr_teorico, 2)))
```
...
prob_teorica <- 1 - pnorm(meta, mean = mu_ventas, sd = sigma_ventas)
porc_teorico <- prob_teorica * 100

ventas_mensuales <- rnorm(n_meses, mean = mu_ventas, sd = sigma_ventas)
porc_simulado <- mean(ventas_mensuales > meta) * 100

print(paste("Porcentaje teórico de cumplimiento:", round(porc_teorico, 2), "%"))
print(paste("Porcentaje simulado en 24 meses:", round(porc_simulado, 2), "%"))
```
...
vida_baterias <- rnorm(n_baterias, mean = mu_vida, sd = sigma_vida)
...
garantia_teorica <- qnorm(1 - prob_cobertura, mean = mu_vida, sd = sigma_vida)

garantia_muestral <- quantile(vida_baterias, 1 - prob_cobertura)

print(paste("Garantía teórica (5º percentil):", round(garantia_teorica, 2), "horas"))
print(paste("Garantía muestral:", round(garantia_muestral, 2), "horas"))
```
...
prob_falla <- pnorm(garantia, mean = mu_vida, sd = sigma_vida)
fallas_esperadas <- total_ventas * prob_falla

set.seed(123)
simulacion_ventas <- rnorm(total_ventas, mean = mu_vida, sd = sigma_vida)
fallas_observadas <- sum(simulacion_ventas < garantia)

print(paste("Probabilidad de falla (< 900h):", round(prob_falla, 4)))
print(paste("Cantidad esperada de fallas (Teórico):", round(fallas_esperadas, 0)))
print(paste("Cantidad de fallas en simulación:", fallas_observadas))
```
...
media_muestral <- mean(vida_baterias)
mediana_muestral <- median(vida_baterias)

cat("Media Teórica:", mu_vida, "\n")
cat("Media Muestral:", round(media_muestral, 2), "\n")
cat("Diferencia Media:", round(abs(mu_vida - media_muestral), 2), "\n\n")

cat("Mediana Teórica:", mu_vida, "\n")
cat("Mediana Muestral:", round(mediana_muestral, 2), "\n")
cat("Diferencia Mediana:", round(abs(mu_vida - mediana_muestral), 2), "\n")
```
...
p10_teorico <- qnorm(0.10, mean = mu_vida, sd = sigma_vida)
p10_muestral <- quantile(vida_baterias, 0.10)

print(paste("Percentil 10 teórico:", round(p10_teorico, 2)))
print(paste("Percentil 10 muestral:", round(p10_muestral, 2)))
```
...
n_mediciones <- 1000
mu_error <- 0
sigma_error <- 2.5 # Asumiendo N(0, 2.5^2)
...
prob_teorica <- pnorm(limite, mean = mu_error, sd = sigma_error) - 
                pnorm(-limite, mean = mu_error, sd = sigma_error)

set.seed(123)
n_mediciones <- 1000
errores <- rnorm(n_mediciones, mean = mu_error, sd = sigma_error)

porc_simulado <- mean(errores > -limite & errores < limite) * 100

print(paste("Porcentaje teórico (entre -5 y 5):", round(prob_teorica * 100, 2), "%"))
print(paste("Porcentaje simulado:", round(porc_simulado, 2), "%"))
```
...
prob_teorica <- 2 * (1 - pnorm(umbral, mean = mu_error, sd = sigma_error))

set.seed(123)
n_mediciones <- 1000
errores <- rnorm(n_mediciones, mean = mu_error, sd = sigma_error)

prob_simulada <- mean(abs(errores) > umbral)

print(paste("Probabilidad teórica (|Error| > 6):", round(prob_teorica, 4)))
print(paste("Probabilidad simulada:", round(prob_simulada, 4)))
```
...
prob_teorica <- pnorm(limite_precision, mean = mu_error, sd = sigma_error) - 
                pnorm(-limite_precision, mean = mu_error, sd = sigma_error)

set.seed(123)
n_mediciones <- 1000
errores <- rnorm(n_mediciones, mean = mu_error, sd = sigma_error)

porc_simulado <- mean(abs(errores) <= limite_precision) * 100

print(paste("Porcentaje teórico de mediciones precisas:", round(prob_teorica * 100, 2), "%"))
print(paste("Porcentaje simulado:", round(porc_simulado, 2), "%"))
```
...
prop_negativos <- mean(errores < 0)
prop_positivos <- mean(errores > 0)

media_sim <- mean(errores)
mediana_sim <- median(errores)

q5 <- quantile(errores, 0.05)
q95 <- quantile(errores, 0.95)
suma_cuantiles <- q5 + q95

print(paste("Proporción < 0:", prop_negativos))
print(paste("Proporción > 0:", prop_positivos))
print(paste("Media:", round(media_sim, 4)))
print(paste("Mediana:", round(mediana_sim, 4)))
print(paste("Q(0.05):", round(q5, 4)))
print(paste("Q(0.95):", round(q95, 4)))
print(paste("Suma Q(0.05) + Q(0.95) (Esperado ~ 0):", round(suma_cuantiles, 4)))

ggplot(data.frame(val = errores), aes(x = val)) +
  geom_histogram(aes(y = after_stat(density)), 
                 bins = 30, 
                 fill = "plum1", 
                 color = "white") +
  stat_function(fun = dnorm, 
                args = list(mean = mu_error, sd = sigma_error), 
                color = "purple", 
                linewidth = 1.2) +
  ...
  labs(
    ...
    subtitle = "La distribución debe estar centrada en la línea roja (0)",
    x = "Error",
    y = "Densidad"
  ) +
  theme_minimal() +
...
head(data.frame(Grupo_A = grupo_a, Grupo_B = grupo_b))
```
...
prob_teorica <- 1 - pnorm(mu_b, mean = mu_a, sd = sigma_a)
porc_teorico <- prob_teorica * 100

porc_simulado <- mean(grupo_a > mu_b) * 100

print(paste("Porcentaje teórico (A > 82):", round(porc_teorico, 2), "%"))
print(paste("Porcentaje simulado (A > 82):", round(porc_simulado, 2), "%"))
```
...
prob_teorica <- pnorm(umbral, mean = mu_b, sd = sigma_b)

set.seed(123)
n <- 200
grupo_b <- rnorm(n, mean = mu_b, sd = sigma_b)
prob_simulada <- mean(grupo_b < umbral)

print(paste("Probabilidad teórica (B < 70):", round(prob_teorica, 4)))
print(paste("Probabilidad simulada:", round(prob_simulada, 4)))
```
...
datos <- data.frame(
  ...
  Grupo = factor(rep(c("Grupo A", "Grupo B"), each = n))
)

medias <- data.frame(
  Grupo = c("Grupo A", "Grupo B"),
  Media = c(mean(grupo_a), mean(grupo_b))
)

ggplot(datos, aes(x = Puntuacion, fill = Grupo)) +
  geom_density(alpha = 0.5, color = NA) +
  geom_vline(data = medias, aes(xintercept = Media, color = Grupo), 
  ...
  labs(
    ...
    subtitle = "Áreas rellenas: Densidad muestral | Líneas punteadas: Teórica",
    x = "Puntuación",
    y = "Densidad"
  ) +
  theme_minimal() +
...
p90_a_teorico <- qnorm(0.90, mean = mu_a, sd = sigma_a)
p90_b_teorico <- qnorm(0.90, mean = mu_b, sd = sigma_b)

p90_a_muestral <- quantile(grupo_a, 0.90)
p90_b_muestral <- quantile(grupo_b, 0.90)

print(paste("Grupo A - Puntuación corte Top 10% (Teórico):", round(p90_a_teorico, 2)))
print(paste("Grupo A - Puntuación corte Top 10% (Simulado):", round(p90_a_muestral, 2)))
print("------------------------------------------------------")
print(paste("Grupo B - Puntuación corte Top 10% (Teórico):", round(p90_b_teorico, 2)))
print(paste("Grupo B - Puntuación corte Top 10% (Simulado):", round(p90_b_muestral, 2)))
```
