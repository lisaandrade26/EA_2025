...
tamanios_n <- c(5, 10, 20, 30, 50, 100)


media_poblacional <- alpha_sim / lambda_sim
varianza_poblacional <- alpha_sim / lambda_sim^2

generar_medias_gamma <- function(n, num_sim, alpha, lambda) {
...
for (i in seq_along(tamanios_n)) {
  n <- tamanios_n[i]
  
  medias_muestrales <- generar_medias_gamma(n, num_simulaciones, alpha_sim, lambda_sim)
  
  media_de_las_medias <- mean(medias_muestrales)
  varianza_de_las_medias <- var(medias_muestrales)
  
  resultados_tcl[[i]] <- data.frame(
    n = n,
    medias = medias_muestrales, 
    media_teorica = media_poblacional,
    media_observada = media_de_las_medias,
    
    var_teorica_xbarra = varianza_poblacional / n,
    var_observada_xbarra = varianza_de_las_medias
  )
}

df_tcl <- do.call(rbind, resultados_tcl)
df_tcl$n_factor <- factor(paste("n =", df_tcl$n), 
                          levels = paste("n =", tamanios_n))

tabla_resumen <- df_tcl %>%
  group_by(n) %>%
  summarise(
    Media_Teorica = mean(media_teorica),
    Media_Simulada = mean(media_observada),
    Diferencia_Media = abs(mean(media_observada) - mean(media_teorica)),
    Var_Teorica_Xbarra = mean(var_teorica_xbarra),
    Var_Simulada_Xbarra = mean(var_observada_xbarra)
  )

kable(tabla_resumen, 
      digits = 4, 
      caption = "Validación del TCL: Comparación de Medias y Varianzas (Gamma 2.5, 1.8)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
...
ggplot(df_tcl, aes(x = medias)) +
  ...
  stat_function(fun = dnorm, 
                args = list(mean = media_poblacional, 
                            sd = sqrt(varianza_poblacional / 5)),
                color = "red", linewidth = 1, linetype = "dashed") +
  
  facet_wrap(~n_factor, scales = "free") +
  
  labs(
    title = "Teorema Central del Límite con Distribución Gamma",
    subtitle = "Evolución de la distribución de la media muestral al aumentar n",
    x = "Media Muestral",
    y = "Densidad"
  ) +
  theme_minimal() +
...
for (n in tamanios_n) {
  ...
  error_pct <- abs(media_observada - mu_teorica) / mu_teorica * 100
  
  resultados_resumen <- rbind(resultados_resumen, data.frame(
    n = n,
    Media_Teorica = mu_teorica,
    Media_Observada_Promedio = media_observada,
    Diferencia_Absoluta = abs(media_observada - mu_teorica),
    Error_Porcentual = error_pct
  ))
}

kable(resultados_resumen, 
      digits = 4, 
      caption = "Comparación: Media Teórica vs. Promedio de Medias Muestrales (Gamma)",
      col.names = c("Tamaño Muestra (n)", "Media Teórica (μ)", 
                    "Media de Medias (x̄)", "Diferencia Abs.", "Error %")) %>%
...
var_poblacional <- alpha / lambda^2

resultados_varianza <- data.frame()

for (n in tamanios_n) {
  medias_muestrales <- replicate(num_simulaciones, mean(rgamma(n, shape = alpha, rate = lambda)))
  
  var_observada <- var(medias_muestrales)
  
  var_teorica_media <- var_poblacional / n
  
  resultados_varianza <- rbind(resultados_varianza, data.frame(
    n = n,
    Var_Poblacional = var_poblacional,
    Var_Teorica_Media = var_teorica_media, 
    
    Var_Observada_Media = var_observada,
    Diferencia = abs(var_observada - var_teorica_media)
  ))
}

kable(resultados_varianza, 
      digits = 5, 
      caption = "Comparación: Varianza de las Medias Muestrales vs. Teoría (σ²/n)",
      col.names = c("n", "Var. Poblacional (σ²)", 
                    "Var. Teórica (σ²/n)", "Var. Observada", "Diferencia Abs.")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

ggplot(resultados_varianza, aes(x = n)) +
  geom_point(aes(y = Var_Observada_Media, color = "Observada"), size = 3) +
  stat_function(fun = function(x) var_poblacional / x, 
                aes(color = "Teórica (σ²/n)"), 
                linewidth = 1) +
  scale_color_manual(name = "Leyenda", 
                     values = c("Observada" = "navy", "Teórica (σ²/n)" = "red")) +
  labs(
    title = "Disminución de la Varianza de la Media con el tamaño de muestra",
    subtitle = "La varianza decae siguiendo la curva 1/n",
    x = "Tamaño de Muestra (n)",
    y = "Varianza de la Media Muestral"
  ) +
  theme_minimal()
```
...
mu_poblacional <- alpha / lambda
sigma_poblacional <- sqrt(alpha / lambda^2)

datos_simulacion <- list()

for (n in tamanios_n) {
  medias <- replicate(n_simulaciones, mean(rgamma(n, shape = alpha, rate = lambda)))
  datos_simulacion[[paste(n)]] <- data.frame(n = n, media_muestral = medias)
}

df_sim <- do.call(rbind, datos_simulacion)

df_sim$n_factor <- factor(df_sim$n, levels = tamanios_n, 
                          labels = paste("n =", tamanios_n))

df_curvas <- list()
rango_x <- seq(min(df_sim$media_muestral), max(df_sim$media_muestral), length.out = 300)

for (n in tamanios_n) {
 
  sigma_xbarra <- sigma_poblacional / sqrt(n)
  
  densidad <- dnorm(rango_x, mean = mu_poblacional, sd = sigma_xbarra)
  
  df_curvas[[paste(n)]] <- data.frame(
    n = n,
    x = rango_x,
    y = densidad,
    n_factor = factor(paste("n =", n), levels = paste("n =", tamanios_n))
  )
}

df_teorico <- do.call(rbind, df_curvas)

ggplot() +

  geom_histogram(data = df_sim, aes(x = media_muestral, y = after_stat(density)),
                 fill = "skyblue4", color = "white", bins = 30, alpha = 0.7) +
  ...
  facet_wrap(~n_factor, scales = "free") +
  labs(
    title = "Verificación Visual del Teorema Central del Límite",
    subtitle = "Histograma de Medias Muestrales (Gamma) vs. Normal Teórica (Roja)",
...
Y_simulada <- replicate(num_simulaciones, sum(rexp(n_sumas, rate = lambda)))

datos_gamma <- data.frame(valor = Y_simulada)

ggplot(datos_gamma, aes(x = valor)) +
  ...
  stat_function(fun = dgamma, 
                args = list(shape = n_sumas, rate = lambda), 
                color = "gray", 
                linewidth = 1.2,
                aes(linetype = "Teórica Gamma(5, 1.5)")) +
  
  scale_linetype_manual(name = "Referencia", values = "solid") +
  labs(
    title = "Suma de Variables Exponenciales",
    subtitle = expression(paste("Histograma de ", Y == sum(X[i]), " donde ", X[i] %~% exp(1.5), " (n=5)")),
    x = "Valor de la Suma (Y)",
    y = "Densidad",
    caption = "El ajuste perfecto confirma que la suma de n exponenciales es una Gamma(n, lambda)"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold"))
```
...
Y_simulada <- replicate(num_simulaciones, sum(rexp(n, rate = lambda)))

datos_grafico <- data.frame(suma = Y_simulada)

ggplot(datos_grafico, aes(x = suma)) +
  ...
  stat_function(fun = dgamma, 
                ...
                color = "orange", 
                linewidth = 1.2,
                aes(linetype = "Teórica")) +
  
  labs(
    title = "Validación: Suma de Exponenciales ~ Gamma",
    subtitle = expression(paste("Histograma de las sumas vs. Densidad Teórica ", Gamma(5, 1.5))),
    x = "Valor de la Suma (Y)",
    y = "Densidad"
  ) +
  theme_minimal() +
...
simular_suma_gamma <- function(n) {

  sumas <- replicate(num_simulaciones, sum(rexp(n, rate = lambda)))
  datos <- data.frame(valor = sumas)

  p <- ggplot(datos, aes(x = valor)) +
    geom_histogram(aes(y = after_stat(density)), 
                   bins = 30, 
                   fill = ifelse(n==10, "aquamarine", "violet"), 
    ...
    labs(
      title = paste("Suma de", n, "Exponenciales"),
      subtitle = expression(paste("Se aproxima a ", Gamma(n, 1.5))),
      x = "Valor de la Suma", 
      y = "Densidad"
    ) +
    theme_minimal()
...
p1 <- simular_suma_gamma(10)
p2 <- simular_suma_gamma(20)

grid.arrange(p1, p2, ncol = 2, 
             top = "Evolución de la Forma: De Asimétrica a Simétrica")
```
